...
[Release](https://github.com/ThirstyKanta/Forwoth/releases/tag/%23release-demo)から最新のものをダウンロードしてください。
> [!IMPORTANT]
...
--
次は**インポート**を説明します。
これは文字通り、セーブデータと同じ形式で記録されたJSONファイルをインポートできます。
（ソフトバージョンが一致していないと不具合を起こす可能性があります。）
...
>[!IMPORTANT]
> 現在はすべての要素を閲覧することが可能です。将来的に編集機能を追加予定です。
...
3.クローンしたJavaプロジェクトのフォルダーを、VSCodeで開いてください
...
**Forwoth**は、就活時を含む自己PRやスキル可視化のために作成しました。
自分の習慣的な学習においての不満があり、
復習のスケジュール管理があまりできていないことが悩みでした。勉強や学習において、記憶ができていないと自信を無くすことは多々あると思います。
ただ、それを正しく復習できていると少しでも自信を持てるようなソフトを開発したい。
...
そこでFlatlafライブラリと、その派生となるライブラリを使用し、**視認性も向上ができた**のは個人的に評価しています。
...
それでも改めてコードを見返すと課題も多く感じられ、無理やりな実装だったり、実装したい機能などに寄り道をしかなり時間をかけてしまったと思います。自分の能力を示す「**一成果物**」としてある程度の形をWIP版としてして公開しています。
